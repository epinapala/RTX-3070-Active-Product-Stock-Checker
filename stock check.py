...
gmail_user = 'androidbrew@gmail.com'
gmail_password = 'Usharani7'
...
def notify(msg):
    ...
    try:
        ...
        server.sendmail(sent_from, to, headers + "\r\n\r\n" + body)
        server.close()
...
def bestBuy():
    for key, value in bestbuy_webcode.items():
        ...
        try:
            x = requests.get("https://api.bestbuy.com/v1/products/" + key + ".json?apiKey=EpYj6rownYJPw29Q6er4yDT5", headers=headers)
            response = json.loads(x.content.decode('utf-8-sig').encode('utf-8'))
            
            if response['onlineAvailability'] == 'true':
...
bbstock = tk.Text(root, width="110", height="22")
